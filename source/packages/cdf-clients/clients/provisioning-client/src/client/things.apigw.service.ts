...
export class ThingsApigwService extends ThingsServiceBase implements ThingsService {
    ...
    async provisionThing(provisioningRequest: ProvisionThingRequest, additionalHeaders?:RequestHeaders): Promise<ProvisionThingResponse> {
        ...
        logger.debug(`[things.apigw.service.provisionThing()] url: ${url}, headers: ${JSON.stringify(headers)}`);
        logger.debug(`[things.apigw.service.provisionThing()] request is: ${JSON.stringify(provisioningRequest)}`);
        const res = await request.post(url)
            .set(headers)
            .send(provisioningRequest);

        logger.debug(`[things.apigw.service.provisionThing()] exiting.`);
        return res.body;
    }

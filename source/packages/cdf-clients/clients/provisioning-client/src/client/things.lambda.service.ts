...
export class ThingsLambdaService extends ThingsServiceBase implements ThingsService {
    ...
    public async provisionThing(provisioningRequest: ProvisionThingRequest, additionalHeaders?:RequestHeaders): Promise<ProvisionThingResponse> {
        ...
        const path = super.thingsRelativeUrl();
        const headers = super.buildHeaders(additionalHeaders);
        logger.debug(`[things.lambda.service.provisionThing()] path: ${path}, headers: ${JSON.stringify(headers)}`);
        logger.debug(`[things.lambda.service.provisionThing()] request is: ${JSON.stringify(provisioningRequest)}`);
        ...
        logger.debug(`[things.lambda.service.provisionThing()] invoking lambda ${this.functionName}`);
        const res = await this.lambdaInvoker.invoke(this.functionName, event);
        logger.debug(`[things.lambda.service.provisionThing()] exiting response: ${JSON.stringify(res)}`);
        return res.body;
    }

...
export class CommandsTemplateCustomResource implements CustomResource {
    ...
    public async create(customResourceEvent: CustomResourceEvent) : Promise<any> {
        ...
        let response;
        try {
             response = await this.lambdaInvoker.invoke(functionName, getEvent);
        ...
        if (!exists) {
           event = new LambdaApiGatewayEventBuilder()
                ...
                .setHeaders(headers)
                .setBody(body);
            response = await this.lambdaInvoker.invoke(functionName, event);

        }
        logger.debug(`CommandsTemplateCustomResource: create: create/update res: ${JSON.stringify(response)}`);

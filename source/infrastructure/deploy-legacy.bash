...
MANDATORY ARGUMENTS:
    ...
    -l (string)   The name of CDF Core stack
...
assetlibrary_invoke_apifunctionname_export="cdf-assetLibrary-${Environment}-restApiFunctionName"
commands_invoke_apifunctionname_export="cdf-commands-${Enviornment}-restApiFunctionName"
provisioning_invoke_apifunctionname_export="cdf-provisioning-${Environment}-restApiFunctionName"
vpcId_export="cdf-network-${ENVIRONMENT}-VPC"
securityGroup_export="cdf-network-${ENVIRONMENT}-SecurityGroup"
privateSubnetOne_export="cdf-network-${ENVIRONMENT}-PrivateSubnetOne"
privateSubnetTwo_export="cdf-network-${ENVIRONMENT}-PrivateSubnetTwo"
...
privateSubnetOne=$(echo ${stack_exports} \
    | jq -r --arg privateSubnetOne_export "$privateSubnetOne_export" \
    '.Exports[] | select(.Name==$privateSubnetOne_export) | .Value'
)

privateSubnetTwo=$(echo ${stack_exports} \
  | jq -r --arg privateSubnetTwo_export "$privateSubnetTwo_export" \
  '.Exports[] | select(.Name==$privateSubnetTwo_export) | .Value'
)

privateSubnets=$(echo "${privateSubnetOne},${privateSubnetTwo}" | sed -e 's/\,/\\\,/')
